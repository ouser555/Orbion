...
uint8_t scroll (void)
  {
    ...
    uint16_t timS;
    static uint16_t hS = 0;
    timS = millis() - hS; //一定會大於10
    if(scr != scr2) //此次滾輪=01或10，清除螢幕顯示計時，可以不用管
      {
        timeoff = LOW;
      }
    if (timS > 10)  //一定滿足
      {
      hS = millis(); //生命週期只到函數結束，所以沒屁用
      if(rot == HIGH) //全域變數，執行過一次就關閉直到滾輪變成11
        {
          ret = scr-scr2;
          if(exo==HIGH) //操作模式時傳滾輪資訊到|USB，目錄模式不會傳
            {
      ...
      if(scr==HIGH && scr2==HIGH) // 此次滾輪=11，下次傳滾輪給USB
        {
          rot=!rot; 
        }
      ...
      if (newPosition != oldPosition)
        {
          ...
          //ret = (newPosition > 0 ? 1:-1);
          //ret = (int8_t)newPosition;
...
void setup() 
  {
    ...
    while(i != 20);  
    display.clearDisplay();
    display.drawBitmap(0, 0, faq, 128, 32, 1);
    display.display();
    Serial.begin(115200);
